...
int main(int argc, char* argv[])
{
    ...
    SPI_Init.TranBits = 8;
    ret = VSI_InitSPI(VSI_USBSPI,0,J8_P2_P4_P6_P8_SPI0_NSS0,&SPI_Init);
    if(ret != ERR_SUCCESS){
    ...
    ret = VSI_WriteReadBytes(VSI_USBSPI,0,J8_P2_P4_P6_P8_SPI0_NSS0,WriteDataTemp,1,ReadDataTemp,3);
    if(ret != ERR_SUCCESS){
    ...
    ret = VSI_WriteBytes(VSI_USBSPI,0,J8_P2_P4_P6_P8_SPI0_NSS0,WriteDataTemp,1);
    if(ret != ERR_SUCCESS){
    ...
    ret = VSI_WriteBytes(VSI_USBSPI,0,J8_P2_P4_P6_P8_SPI0_NSS0,WriteDataTemp,4);
    if(ret != ERR_SUCCESS){
    ...
    do{
        ...
        ret = VSI_WriteReadBytes(VSI_USBSPI,0,J8_P2_P4_P6_P8_SPI0_NSS0,WriteDataTemp,1,ReadDataTemp,1);
    ...
    ret = VSI_WriteBytes(VSI_USBSPI,0,J8_P2_P4_P6_P8_SPI0_NSS0,WriteDataTemp,256+4);
    if(ret != ERR_SUCCESS){
    ...
    WriteDataTemp[2] = 0x00;
    WriteDataTemp[3] = 0x00;
    ret = VSI_WriteReadBytes(VSI_USBSPI,0,J8_P2_P4_P6_P8_SPI0_NSS0,WriteDataTemp,4,ReadDataTemp,256);
    if(ret != ERR_SUCCESS){
    ...
    for(j=1;j<12;j++)
    {
        ...
        SPI_Init.SelPolarity = 0;
        SPI_Init.TranBits = 8;
        ret = VSI_InitSPI(VSI_USBSPI,0,j,&SPI_Init);
        if(ret != ERR_SUCCESS){
            printf("Initialization device error :%d\n",ret);
            return ret;
        }else{
        ...
        // JEDEC ID
        WriteDataTemp[0] = 0x9F;
        ret = VSI_WriteReadBytes(VSI_USBSPI,0,j,WriteDataTemp,1,ReadDataTemp,3);
        if(ret != ERR_SUCCESS){
            printf("Read flash ID error :%d\n",ret);
            return ret;
        }else{
        ...
        // Write Enable
        WriteDataTemp[0] = 0x06;
        ret = VSI_WriteBytes(VSI_USBSPI,0,j,WriteDataTemp,1);
        if(ret != ERR_SUCCESS){
            printf("Flash write enable error :%d\n",ret);
            return ret;
        }else{
        ...
        WriteDataTemp[2] = 0x00;
        WriteDataTemp[3] = 0x00;
        ret = VSI_WriteBytes(VSI_USBSPI,0,j,WriteDataTemp,4);
        if(ret != ERR_SUCCESS){
            printf("Sector Erase start error :%d\n",ret);
            return ret;
        }else{
        ...
        do{
            WriteDataTemp[0] = 0x05;// Read Status Register-1
            ret = VSI_WriteReadBytes(VSI_USBSPI,0,j,WriteDataTemp,1,ReadDataTemp,1);
        ...
        ret = VSI_WriteBytes(VSI_USBSPI,0,j,WriteDataTemp,256+4);
        if(ret != ERR_SUCCESS){
            printf("Page program start error :%d\n",ret);
            return ret;
        }else{
        ...
        WriteDataTemp[2] = 0x00;
        WriteDataTemp[3] = 0x00;
        ret = VSI_WriteReadBytes(VSI_USBSPI,0,j,WriteDataTemp,4,ReadDataTemp,256);
        if(ret != ERR_SUCCESS){
            printf("Read Data error :%d\n",ret);
            return ret;
        }else{
