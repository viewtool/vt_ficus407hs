...
typedef struct {
  ...
  uint32_t       xSize;       // Native xSize of window to be zoomed in pixels
  uint32_t       ySize;       // Native ySize of window to be zoomed in pixels
  uint32_t       xSizeParent; // xSize of parent window
  uint32_t       ySizeParent; // ySize of parent window
...
uint32_t  WM_SetCreateFlags(uint32_t Flags);
...
WM_HWIN WM_CreateWindow              (int x0, int y0, int xSize, int ySize, uint32_t Style, WM_CALLBACK * cb, int NumExtraBytes);
WM_HWIN WM_CreateWindowAsChild       (int x0, int y0, int xSize, int ySize, WM_HWIN hWinParent, uint32_t Style, WM_CALLBACK* cb, int NumExtraBytes);
...
void     WM_MOTION_SetMoveable     (WM_HWIN hWin, uint32_t Flags, int OnOff);
...
/* Static memory devices */
  ...
  int               GUI_MEMDEV_BlendWinBk       (WM_HWIN hWin, int Period, uint32_t BlendColor, U8 BlendIntens);
  int               GUI_MEMDEV_BlurAndBlendWinBk(WM_HWIN hWin, int Period, U8 BlurDepth, uint32_t BlendColor, U8 BlendIntens);
