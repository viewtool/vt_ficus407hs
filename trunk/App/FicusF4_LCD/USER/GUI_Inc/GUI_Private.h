...
void              GUI_MEMDEV__BlendColor32       (GUI_MEMDEV_Handle hMem, uint32_t BlendColor, U8 BlendIntens);
...
extern const uint32_t GUI_Pow10[10];
...
int      GUI__GetAlphaBuffer    (uint32_t ** ppCurrent, uint32_t ** ppConvert, uint32_t ** ppData, int * pVXSizeMax);
int      GUI__AllocAlphaBuffer  (int AllocDataBuffer);
uint32_t    * GUI__DoAlphaBlending   (int x, int y, uint32_t * pData, int xSize, tLCDDEV_Index2Color * pfIndex2Color_DEV, int * pDone);
...
int        GUI_XBF__GetOff       (const GUI_XBF_DATA * pXBF_Data, unsigned c, uint32_t * pOff);
int        GUI_XBF__GetOffAndSize(const GUI_XBF_DATA * pXBF_Data, unsigned c, uint32_t * pOff, U16 * pSize);
...
/* Conversion routines */
void GUI_AddHex     (uint32_t v, U8 Len, char ** ps);
void GUI_AddBin     (uint32_t v, U8 Len, char ** ps);
...
void GUI__AddSpaceHex      (uint32_t v, U8 Len, char ** ps);
...
uint32_t GUI__Read32(const U8 ** ppData);
...
GUI_TIMER_HANDLE GUI_TIMER__GetNextTimer   (GUI_TIMER_HANDLE hTimer, uint32_t * pContext);
GUI_TIMER_HANDLE GUI_TIMER__GetFirstTimer  (uint32_t * pContext);
GUI_TIMER_HANDLE GUI_TIMER__GetNextTimerLin(GUI_TIMER_HANDLE hTimer, uint32_t * pContext);
...
void      GUI__MixColorsBulk   (uint32_t * pFG, uint32_t * pBG, uint32_t * pDst, unsigned OffFG, unsigned OffBG, unsigned OffDest, unsigned xSize, unsigned ySize, U8 Intens);
...
//
extern void (* GUI__pfMixColorsBulk)(uint32_t * pFG, uint32_t * pBG, uint32_t * pDst, unsigned OffFG, unsigned OffBG, unsigned OffDest, unsigned xSize, unsigned ySize, U8 Intens);
